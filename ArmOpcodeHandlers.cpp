...
ARM7TDMI::Cycles ARM7TDMI::ArmOpcodeHandlers::sdtHandler(uint32_t instruction,
    ...
    uint32_t address;
    bool u =
        dataTransGetU(instruction);  // U - Up/Down Bit (0=down; subtract offset
                                     // from base, 1=up; add to base)
    bool p =
        dataTransGetP(instruction);  // P - Pre/Post (0=post; add offset after
                                     // transfer, 1=pre; before trans.)

    if (p) {  // add offset before transfer
        address = u ? address + offset : address - offset;
        if (dataTransGetW(
                instruction)) {  // write address back into base register
